...
df = pd.DataFrame(columns=['Event_Year','Event_Julday','Event_Hour','Event_Min','Event_Sec',
                            ...
                            'Component','Network','Distance','SNR'])
...
for f in event_folders:
    ...
    for j in emp_staname:
        ...
        if deg >= 20:
            ...
            if maximum_p_wind <= 1:
                bad = 'bad'
                m.write("%s %5.5f %5.7f %5.7f %5.7f %5.4f %s\n" % (event_file, float(maximum_p_wind), float(tr.stats.sac.user8), ray_parameter, ray_incidence, float(tr.stats.sac.gcarc), bad))
                #m.write("%s %5.5f %5.7f %5.7f %5.7f %5.4f %s\n" % (event_file, float(maximum_p_wind), float(tr.stats.sac.user8), ray_parameter, ray_incidence, float(tr.stats.sac.gcarc), bad))
            elif maximum_p_wind > 1 and maximum_p_wind <= 2:
                ok_bad = 'ok_bad'
                m.write("%s %5.5f %5.7f %5.7f %5.7f %5.4f %s\n" % (event_file, float(maximum_p_wind), float(tr.stats.sac.user8), ray_parameter, ray_incidence, float(tr.stats.sac.gcarc), ok_bad))
                #m.write("%s %5.5f %5.7f %5.7f %5.7f %5.4f %s\n" % (event_file, float(maximum_p_wind), float(tr.stats.sac.user8), ray_parameter, ray_incidence, float(tr.stats.sac.gcarc), ok_bad))
            elif maximum_p_wind > 2 and maximum_p_wind <= 3:
                ok_good = 'ok_good'
                m.write("%s %5.5f %5.7f %5.7f %5.7f %5.4f %s\n" % (event_file, float(maximum_p_wind), float(tr.stats.sac.user8), ray_parameter, ray_incidence, float(tr.stats.sac.gcarc), ok_good))
                #m.write("%s %5.5f %5.7f %5.7f %5.7f %5.4f %s\n" % (event_file, float(maximum_p_wind), float(tr.stats.sac.user8), ray_parameter, ray_incidence, float(tr.stats.sac.gcarc), ok))
            elif maximum_p_wind > 3:
                good = 'good'
                m.write("%s %5.5f %5.7f %5.7f %5.7f %5.4f %s\n" % (event_file, float(maximum_p_wind), float(tr.stats.sac.user8), ray_parameter, ray_incidence, float(tr.stats.sac.gcarc), good))
                #m.write("%s %5.5f %5.7f %5.7f %5.7f %5.4f %s\n" % (event_file, float(maximum_p_wind), float(tr.stats.sac.user8), ray_parameter, ray_incidence, float(tr.stats.sac.gcarc), ok))
